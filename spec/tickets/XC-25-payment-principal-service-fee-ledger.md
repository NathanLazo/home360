...
plataforma y principal. Se necesita un ledger congelado por pago; la única decisión que este
ticket no toma es si la tarifa plana se devuelve en un reembolso parcial. Ese gate bloquea
F3-05, no el schema ni los helpers de XC-25, por lo que F0 sigue implementable.
...
- Fuera de alcance: agregados y UI (`XC-27`), implementar servicios F3, elegir la política
  de refund de la tarifa, cambiar precios de planes o ejecutar migraciones/Stripe.
...
4. F3-05 `refundPayment` persiste la asignación explícita entre principal y tarifa. La suma debe
   coincidir con el Refund de Stripe. El caller no puede pasar un monto opaco.
...
Roger registra la política de reembolso de la tarifa antes de F3-05.
