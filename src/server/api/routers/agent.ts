...
const updateSchema = z
  ...
  .refine((input) => input.title !== undefined || input.messages !== undefined, {
...
const agentProcedure = protectedProcedure.use(({ ctx, next }) => {
  ...
  return next({ ctx: { ...ctx, agent: { area, userId: ctx.session.user.id } } });
});

function unexpectedFailure(error: unknown, message: string): TrpcResponse<never> {
  const normalized = normalizeError(error);
  return fail(normalized.code, normalized.status, message);
}

function asJson(messages: z.infer<typeof messagesSchema>): Prisma.InputJsonValue {
  return JSON.parse(JSON.stringify(messages)) as Prisma.InputJsonValue;
}

export const agentRouter = createTRPCRouter({
  getArea: agentProcedure.query(
    async ({ ctx }): Promise<TrpcResponse<{ area: AgentArea; readOnly: boolean }>> =>
      ok(
        { area: ctx.agent.area, readOnly: ctx.session.user.impersonator !== null },
        "Agent area resolved",
      ),
  ),
  ...
  getConversation: agentProcedure
    ...
    .query(async ({ ctx, input }): Promise<TrpcResponse<AgentConversationDetail>> => {
      try {
        const result = await getAgentConversation(ctx.db, ctx.agent, input.id);
  ...
  createConversation: agentProcedure
    .input(createSchema)
    .mutation(async ({ ctx, input }): Promise<TrpcResponse<AgentConversationSummary>> => {
      try {
        const conversation = await createAgentConversation(ctx.db, ctx.agent, {
          title: input.title,
  ...
  deleteConversation: agentProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<TrpcResponse<{ id: string }>> => {
      try {
        const result = await deleteAgentConversation(ctx.db, ctx.agent, input.id);
