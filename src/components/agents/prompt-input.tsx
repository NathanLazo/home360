      ...
      <div className="mt-1 flex min-h-8 items-center gap-1">
        {actions.length ? (
          <MorphPopover open={actionsOpen} onOpenChange={setActionsOpen}>
            <MorphPopoverTrigger>
              <Button type="button" variant="ghost" size="icon" disabled={(disabled ?? false) || loading} aria-label="Add to prompt">
                <motion.span
                  aria-hidden="true"
            ...
            <SelectTrigger className="h-8 w-auto max-w-52 rounded-xl border-0 bg-transparent px-2 py-0 text-xs hover:bg-muted focus-visible:ring-2">
              <span className="flex min-w-0 items-center gap-1.5">
                ...
                <span className="truncate text-muted-foreground">
                  {currentModel?.label ?? chooseModelLabel}
                </span>
              </span>
            </SelectTrigger>
            <SelectContent className="right-auto z-50 w-52 shadow-lg">
              {models.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
        ...
        <Button type={loading ? "button" : "submit"} size="icon" disabled={loading ? !onStop : !canSubmit} aria-label={loading ? stopLabel : submitLabel} onClick={loading ? onStop : undefined} className="ml-auto">
          <AnimatePresence initial={false} mode="popLayout">
            <motion.span
              key={submitIndicator ? "custom" : loading ? "stop" : "send"}
