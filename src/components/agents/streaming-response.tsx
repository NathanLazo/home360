...
const DEFAULT_LABELS: StreamingResponseLabels = {
  copy: "Copy response",
  copied: "Copied",
...
export interface StreamingResponseProps {
  ...
  /** Visible/aria copy of the action bar; the host translates it. */
  labels?: Partial<StreamingResponseLabels>;
}
...
export function StreamingResponse({
  ...
  const canCopy = Boolean(copyText) || Boolean(onCopy);
  const hasSources = sources.length > 0;
  const shouldShowActions =
    showActions && !streaming && (canCopy || Boolean(onRetry) || complete || hasSources);
        ...
        {shouldShowActions ? (
          ...
          >
            <div className={cn("flex items-center gap-0.5", actionsClassName)}>
              ...
              {onRetry ? (
                <ResponseAction label={labels.retry} onClick={onRetry}>
