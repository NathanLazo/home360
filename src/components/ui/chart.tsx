...
function getStringKey(value: unknown, fallback = "value") {
...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload;
    const key = getStringKey(labelKey ?? item?.dataKey ?? item?.name);
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            const key = getStringKey(nameKey ?? item.name ?? item.dataKey);
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
  ...
  return (
    ...
    >
      {payload
        .filter((item) => item.type !== "none")
        .map((item, index) => {
          const key = getStringKey(nameKey ?? item.dataKey);
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
